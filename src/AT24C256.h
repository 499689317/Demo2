...
// 分页读写相当于批量读写字节，at24c256可以分成512页，每页64字节，len长度可以超过255字节长度，在写入时做了分页处理
...
extern void AT24C256WritePage(unsigned short addr, const unsigned char* data, unsigned short len);
// 页读，len为需要读取多少个字节，每读取一个字节，长度len减1直到为0说明读完了
// 注：读取完一个字节，发送应答信号为0表示继续读取，直到读完数据发送ack=1表示不继续读了
extern void AT24C256ReadPage(unsigned short addr, unsigned char* data, unsigned short len);
